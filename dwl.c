...
static int drawstatus(Monitor *m, int traywidth);
...
remove_outer_separators(char **str)
{
	...
	char *temp_tag_name = ecalloc(clean_tag_name_len + 1, sizeof(char));
...
{
	char *temp_tag_name = ecalloc(new_size, sizeof(char));
...
drawbar(Monitor *m)
{
	...
	memset(icons_per_tag, 0, LENGTH(tags) * sizeof(unsigned int));

	for (size_t tag_idx = 0; tag_idx < LENGTH(tags); tag_idx++) {
		/* reset each tag to its default label, freeing any appicon text */
		free(m->tag_icons[tag_idx]);
...
int
drawstatus(Monitor *m, int traywidth)
{
	int x, tw, iw;
	char rstext[512] = "";
...
rendermon(struct wl_listener *listener, void *data)
{
	...
	/* wlroots >= 0.19 tracks intermediate surface states internally (scene
	 * damage + explicit sync), so there is no need to delay rendering until
	 * every client acknowledges its pending resize. Doing so could leave
	 * c->resize stuck non-zero when a background client never commits,
	 * freezing the output until restart (dwl issue #1203). */
...
updatetitle(struct wl_listener *listener, void *data)
{
	...
	/* appicon is set once at window creation via applyrules(); do NOT
	 * re-evaluate it on title change, so the dwl bar tag keeps the terminal
	 * icon instead of switching to the app run inside tmux (app icons live
	 * in the tmux status bar, not the dwl bar) */
	if (c == focustop(c->mon))
		drawbars();
}
